...
export function CheckoutResumo() {
  ...
  // Calcula o total
  const subtotal = pedido.itens.reduce((total, item) => total + (item.preco * item.qtd), 0);
  const frete = 15.90; // Valor mockado - substitua pelo cálculo real
  ...
  return (
    <Container>
      ...
      <Section>
        ...
        {pedido.endereco ? (
          <AddressInfo>
            <h4>{pedido.endereco.tipo}</h4>
            <p>{pedido.endereco.logradouro}, {pedido.endereco.numero}</p>
            <p>{pedido.endereco.complemento && `${pedido.endereco.complemento} - `}{pedido.endereco.bairro}</p>
      ...
      <Section>
        ...
        {pedido.pagamento ? (
          <PaymentInfo>
            <h4>{pedido.pagamento.tipo === 'cartao' ? 'Cartão de Crédito' : 'Boleto Bancário'}</h4>
